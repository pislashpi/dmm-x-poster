...
class SchedulerService:
    ...
    def generate_post_text(self, product, custom_text=None):
        """投稿テキストを生成"""
        # カスタムテキストがあれば使用
        if custom_text:
            text = custom_text
        ...
        # URLを追加（X(Twitter)が自動的に短縮）
        if product.url:
            text += f"\n{product.url}"
        
        return text

    def create_post(self, product_id, scheduled_date=None, custom_text=None):
        """投稿を作成"""
        ...
        # 投稿テキストを生成
        post_text = self.generate_post_text(product, custom_text)
        
        # 次の投稿時間を計算
        next_time = scheduled_date
        if not next_time:
            next_time = self.calculate_next_post_time()
        elif isinstance(next_time, str):
            try:
                next_time = datetime.fromisoformat(next_time)
            except ValueError:
                logger.warning(f"Invalid date format: {scheduled_date}, using calculated time")
                next_time = self.calculate_next_post_time()
    ...
    def create_immediate_post(self, product_id, custom_text=None):
        """即時投稿を作成して実行"""
        product = Product.query.get(product_id)
        if not product:
            logger.error(f"Product not found: {product_id}")
